...
#include "mcs/ubuntu/unity.h"
#include "mcs/ubuntu/unitydisplaylock.h"
...
void UnityDisplayLock::Acquire(DisplayState state) {
    ...
    g_dbus_connection_call(connection_.get(),
        ...
        auto thiz = static_cast<mcs::SharedKeepAlive<UnityDisplayLock>*>(user_data)->ShouldDie();

        GError *error = nullptr;
        auto result = g_dbus_connection_call_finish(thiz->connection_.get(), res, &error);
        if (!result) {
            MCS_ERROR("Failed to acquire display lock: %s", error->message);
            g_error_free(error);
            return;
        }
        ...
        MCS_DEBUG("Successfully locked display (cookie %d)", thiz->cookie_);

    }, new mcs::SharedKeepAlive<UnityDisplayLock>{shared_from_this()});
}
...
void UnityDisplayLock::ReleaseInternal() {
    ...
    g_dbus_connection_call(connection_.get(),
        ...
        MCS_DEBUG("Successfully unlocked display");
