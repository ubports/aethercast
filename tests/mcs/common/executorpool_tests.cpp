...
TEST(ExecutorPool, PoolSizeIsRespected) {
    ...
    mcs::common::ExecutorPool pool(factory, 1);
...
TEST(ExecutorPool, StartAndStopsAllExecutors) {
    ...
    mcs::common::ExecutorPool pool(factory, 2);
...
TEST(ExecutorPool, RecoversFromFailedExecutorStart) {
    ...
    mcs::common::Executable::Ptr executable0 = std::make_shared<MockExecutable>();

    EXPECT_CALL(*factory, Create(executable0))
            .Times(1)
            .WillOnce(Return(executor0));

    mcs::common::Executable::Ptr executable1 = std::make_shared<MockExecutable>();
