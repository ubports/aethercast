...
class MockRendererReport : public mcs::video::RendererReport {
public:
...
public:
    ...
    void ExpectValidConfiguration() {
        mcs::video::DisplayOutput output_mode{mcs::video::DisplayOutput::Mode::kExtend, 1280, 720, 30};

        EXPECT_CALL(*mock_buffer_producer, OutputMode())
                .WillRepeatedly(Return(output_mode));

        mcs::video::BaseEncoder::Config encoder_config{};
...
TEST_F(StreamRendererFixture, TakesBufferFromProducerAndForwardsToEncoder) {
    ...
    mcs::video::Buffer::Ptr output_buffer;
...
TEST_F(StreamRendererFixture, CorrectBufferManagement) {
    ...
    auto buffers = mcs::video::BufferQueue::Create(renderer->BufferSlots());

    running = true;
    iteration_count = 0;

    EXPECT_CALL(*mock_encoder, QueueBuffer(_))
            .WillRepeatedly(Invoke([&](const mcs::video::Buffer::Ptr &buffer) {
                buffers->Push(buffer);
            }));
