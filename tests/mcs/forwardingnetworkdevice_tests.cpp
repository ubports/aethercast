...
#include <mcs/forwardingnetworkdevice.h>

#include "mock_network_device.h"

TEST(ForwardingNetworkDevice, ThrowsForNullptrOnConstruction) {
    EXPECT_THROW(mcs::ForwardingNetworkDevice(mcs::NetworkDevice::Ptr{}), std::logic_error);
}
...
TEST(ForwardingNetworkDevice, ForwardsCallsToImpl) {
    ...
    mcs::ForwardingNetworkDevice fmc{impl};
