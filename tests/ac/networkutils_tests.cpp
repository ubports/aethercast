...
TEST(NetworkUtils_PickRandomPort, PicksPortsInRange) {
    for (int n = 0; n < 100; n++) {
        auto port = ac::NetworkUtils::PickRandomPort();
        EXPECT_LE(ac::NetworkUtils::kMinUserPort, port);
        EXPECT_GE(ac::NetworkUtils::kMaxUserPort, port);
    }
}
...
TEST(NetworkUtils_MakeSocketNonBlocking, SocketFlagsCorrectlySet) {
    ...
    EXPECT_EQ(0, ac::NetworkUtils::MakeSocketNonBlocking(sock));
