...
public:
    MOCK_METHOD1(OnBufferAvailable, void(const ac::video::Buffer::Ptr&));
    MOCK_METHOD1(OnBufferWithCodecConfig, void(const ac::video::Buffer::Ptr&));
    MOCK_METHOD0(OnBufferReturned, void());
};

class MockBufferDelegate : public ac::video::Buffer::Delegate {
public:
    MOCK_METHOD1(OnBufferFinished, void(const ac::video::Buffer::Ptr&));
};
...
public:
    H264EncoderFixture() :
        mock_report(std::make_shared<ac::test::android::MockEncoderReport>()) {
    }

    void ExpectValidConfiguration(const ac::video::BaseEncoder::Config &config, const ac::test::android::MockMedia::Ptr &mock) {
    ...
    void ExpectValidStartAndStop(const ac::test::android::MockMedia::Ptr &mock) {
    ...
    std::shared_ptr<ac::test::android::MockEncoderReport> mock_report;
...
TEST_F(H264EncoderFixture, ValidDefaultConfiguration) {
    auto encoder = ac::android::H264Encoder::Create(mock_report);
...
TEST_F(H264EncoderFixture, MediaMessageCreationFails) {
    auto mock = std::make_shared<ac::test::android::MockMedia>();
...
TEST_F(H264EncoderFixture, ReturnsPackedBufferAndReleaseProperly) {
    ...
    auto input_buffer = ac::video::Buffer::Create(anwb);
...
TEST_F(H264EncoderFixture, QueueBufferDoesNotCrashWhenInactive) {
    ...
    auto buffer = ac::video::Buffer::Create(nullptr);
...
TEST_F(H264EncoderFixture, ExecuteProvidesBuffers) {
    ...
    ac::TimestampUs input_buffer_timestamp = 23ll;
